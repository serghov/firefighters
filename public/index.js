...
clouds.on(R.CONST.READY, (evt) => {
    ...
    evt.target.position.copy(area.position);
...
R.messenger.once(R.CONST.ALL_SCULPTS_READY, () => {
    ...
    const light = new THREE.DirectionalLight(0xffffff, 1.2, 200);
...
// Add Sun Helper
    ...
    R.messenger.once(R.CONST.ALL_SCULPTS_READY, () => {
        ...
        sky.on(R.CONST.GAMEPAD_MOVE, (e) => {
            if (sky.engaged) {
                let angle = area.getAngle(e.point, "x", "y");
                if (angle < 0) {
                    angle += 2 * Math.PI
                }
